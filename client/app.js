    ...
    openAuthModal(mode = 'login') {
        ...
        this.openModal(`
            <div style="display:flex; width:100%; height:100%; background:#ffffff; box-sizing:border-box; overflow:hidden; position:relative; font-family:'Inter', -apple-system, BlinkMacSystemFont, sans-serif;">
                ...
                <!-- LEFT COLUMN: AUTHENTICATION FORM PANEL (PERFECT COMPACT UNIFORM LAYOUT FOR ZERO OVERFLOW) -->
                <div style="flex:1.05; background:#f8fafc; padding:1.2rem 2.2rem 1.1rem; display:flex; flex-direction:column; justify-content:space-between; box-sizing:border-box; overflow:hidden; height:100%;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; width:100%; flex-shrink:0;">
                        ...
                        <div style="margin-right:2.5rem;">
                            ${isRegister ? `
                                ...
                                <button type="button" onclick="app.openAuthModal('login')" style="border:1px solid #cbd5e1; border-radius:6px; padding:0.3rem 0.85rem; font-weight:700; background:#ffffff; color:#0f172a; font-size:0.78rem; cursor:pointer; margin-left:0.35rem; transition:all 0.2s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='#ffffff'">Login</button>
                            ` : ''}
                            ${isLogin ? `
                    ...
                    <div style="flex:1; display:flex; flex-direction:column; justify-content:center; max-width:360px; width:100%; margin:0 auto; box-sizing:border-box; padding:0.2rem 0;">
                        ...
                        <div id="authFormAlert" style="display:none; margin-bottom:0.35rem; font-size:0.74rem;"></div>
                        <div id="authAlertBox" style="display:none; margin-bottom:0.35rem; font-size:0.74rem;"></div>

                        ${!isForgot ? `
                            <!-- SOCIAL LOGIN BUTTONS -->
                            <button type="button" onclick="app.loginWithGoogle()" style="width:100%; height:35px; border:1px solid #e2e8f0; border-radius:8px; background:#ffffff; display:flex; align-items:center; justify-content:center; gap:0.45rem; cursor:pointer; font-weight:700; font-size:0.8rem; color:#0f172a; margin-bottom:0.35rem; transition:all 0.2s; flex-shrink:0;">
                            ...
                            <button type="button" onclick="app.showToast('Apple Sign-In feature available soon', 'info')" style="width:100%; height:35px; border:1px solid #e2e8f0; border-radius:8px; background:#ffffff; display:flex; align-items:center; justify-content:center; gap:0.45rem; cursor:pointer; font-weight:700; font-size:0.8rem; color:#0f172a; margin-bottom:0.4rem; transition:all 0.2s; flex-shrink:0;">
                        ...
                        <form onsubmit="event.preventDefault(); app.handleAuthSubmit('${mode}');" style="display:flex; flex-direction:column; flex-shrink:0;">
                            ${isRegister ? `
                                <!-- FULL NAME -->
                                <input type="text" id="authName" required placeholder="Full name" style="width:100%; height:35px; border:1px solid #e2e8f0; border-radius:8px; padding:0 0.8rem; font-size:0.82rem; box-sizing:border-box; margin-bottom:0.4rem; background:#ffffff; color:#0f172a; font-weight:500;">

                                <!-- EMAIL ADDRESS WITH INLINE OTP BUTTON -->
                                <div style="position:relative; margin-bottom:0.4rem;">
                                    <input type="email" id="authEmail" required placeholder="Email address" style="width:100%; height:35px; border:1px solid #e2e8f0; border-radius:8px; padding:0 5rem 0 0.8rem; font-size:0.82rem; box-sizing:border-box; background:#ffffff; color:#0f172a; font-weight:500;">
                                    <button type="button" id="btnSendOtp" onclick="app.sendSignupOtp()" style="position:absolute; right:3px; top:3px; height:29px; padding:0 0.65rem; background:#2b5e48; color:#ffffff; border:none; border-radius:6px; font-size:0.72rem; font-weight:700; cursor:pointer;">
                                        Send OTP
                                    </button>
                                </div>
                                ...
                                <div style="position:relative; margin-bottom:0.5rem;">
                                    <input type="password" id="authPassword" required placeholder="Create password" style="width:100%; height:35px; border:1px solid #e2e8f0; border-radius:8px; padding:0 2rem 0 0.8rem; font-size:0.82rem; box-sizing:border-box; background:#ffffff; color:#0f172a;" onkeyup="app.checkPasswordStrength(this.value)">
                                    <button type="button" id="eyeRegPass" onclick="app.togglePasswordVisibility('authPassword', 'eyeRegPass')" style="position:absolute; right:0.65rem; top:7px; background:none; border:none; cursor:pointer; font-size:0.9rem; color:#64748b;">👁️</button>
                                </div>
                            ` : `
                                <!-- LOGIN EMAIL -->
                                <input type="text" id="authEmail" required placeholder="Email address" style="width:100%; height:36px; border:1px solid #e2e8f0; border-radius:8px; padding:0 0.8rem; font-size:0.82rem; box-sizing:border-box; margin-bottom:0.45rem; background:#ffffff; color:#0f172a; font-weight:500;">

                                <!-- LOGIN PASSWORD -->
                                <div style="position:relative; margin-bottom:0.5rem;">
                                    <input type="password" id="authPassword" required placeholder="Password" style="width:100%; height:36px; border:1px solid #e2e8f0; border-radius:8px; padding:0 2rem 0 0.8rem; font-size:0.82rem; box-sizing:border-box; background:#ffffff; color:#0f172a;">
                ...
                <!-- RIGHT COLUMN: SACRED HERO PHOTO WITH TITLE & SUBTITLE IN ELEGANT GLASS CARD -->
                <div style="flex:1; position:relative; overflow:hidden; height:100%;">
                    ${isRegister ? `
                        ...
                        <div style="position:absolute; inset:0; background: url('https://images.pexels.com/photos/18996760/pexels-photo-18996760.jpeg') center center / cover no-repeat; padding:2.2rem; display:flex; flex-direction:column; justify-content:flex-end; color:#ffffff; box-sizing:border-box;">
                            <div style="position:absolute; inset:0; background:linear-gradient(180deg, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.85) 100%);"></div>
                            ...
                            <div style="position:relative; z-index:2; background:rgba(15, 23, 42, 0.65); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px); border-radius:16px; padding:1.5rem; border:1px solid rgba(255, 255, 255, 0.2); max-width:400px; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
                                <h2 style="font-size:1.65rem; font-weight:900; color:#ffffff; line-height:1.2; margin:0 0 0.45rem 0; letter-spacing:-0.02em;">
                                    Create your account
                                </h2>
                                <p style="font-size:0.84rem; color:rgba(255,255,255,0.92); line-height:1.5; font-weight:400; margin:0;">
                                    Get started with your spiritual journey in minutes. Join thousands of pilgrims who trust our premium services.
                                </p>
                            </div>
                    ...
                    ` : `
                        ...
                        <div style="position:absolute; inset:0; background: url('https://images.pexels.com/photos/35315919/pexels-photo-35315919.jpeg') center center / cover no-repeat; padding:2.2rem; display:flex; flex-direction:column; justify-content:flex-end; color:#ffffff; box-sizing:border-box;">
                            ...
                            <div style="position:relative; z-index:2; background:rgba(15, 23, 42, 0.65); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px); border-radius:16px; padding:1.5rem; border:1px solid rgba(255, 255, 255, 0.2); max-width:400px; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
                                ...
                                <p style="font-size:0.84rem; color:rgba(255,255,255,0.92); line-height:1.5; font-weight:400; margin:0;">
                                    Sign in to continue where you left off. Experience peace of mind with our premium pilgrimage services.
                                </p>
                            </div>
